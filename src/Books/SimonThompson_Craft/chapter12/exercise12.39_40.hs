...
-- exercise 40 --------------------------------------------------------------------
-- NOTE --> exercise 40 - make sure to change non-proper words to lowercase.
-- note splits a string into list of lines at \n
-- precondition: assume it doesn't start or end with \n character.
lines :: Doc -> [Line]
lines text = Prelude.lines (map toLower text)
-----------------------------------------------------------------------------------
...
-- exercise 39 ---------------------------------------------------------------------
numOccurs :: [([Int], Word)] -> [(Int, Word)]
numOccurs ps = map occur ps
    where occur (ns, w) = (length ns, w)

-- returns number of occurrences next to the word, instead of lines where it occurred.
makeIndexOccurs :: Doc -> [(Int, Word)]
makeIndexOccurs = numOccurs   .
                  amalgamate  .
                  makeLists   .
                  sortD       .
                  allNumWords .
